...
public class AnimalMonitorImpl implements AnimalMonitor {
  ...
  public void printList() {
    for (int i = 0; i < sightings.size(); i++) {
      System.out.println(sightings.get(i));
    }
  ...
  public void printSightingsOf(String animal) {
    for (int i = 0; i < sightings.size(); i++) {
      if (animal == sightings.get(i).getAnimal()) {
        System.out.println(sightings.get(i));
      }
  ...
  public void printSightingsBy(int spotter) {
    for (int i = 0; i < sightings.size(); i++) {
      if (spotter == sightings.get(i).getSpotter()) {
        System.out.println(sightings.get(i));
      }
  ...
  public void printEndangered(List<String> animalNames, int dangerThreshold) {
    ...
    for (int i = 0; i < sightings.size(); i++) {
      if (sightings.get(i).getCount() <= dangerThreshold) {
        System.out.println(sightings.get(i));
      }
    }
  }
  ...
  public List<Sighting> printSightingsInPeriod(int period) {
    List<Sighting> ret = new ArrayList<>();
    for (int i = 0; i < sightings.size(); i++) {
      if (period == sightings.get(i).getPeriod()) {
        System.out.println(sightings.get(i));
        ret.add(sightings.get(i));
      }
    }
    return ret;
  }
  ...
  public List<Sighting> printCounts(String animal) {
    ...
    for (int i = 0; i < sightings.size(); i++) {
      if (animal == sightings.get(i).getAnimal()) {
        sum += sightings.get(i).getCount();
      }
    }
    System.out.println("目击数量："+sum);
    return null;
  }
